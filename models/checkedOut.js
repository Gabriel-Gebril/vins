...
exports.createBulk = function(obj,callback){
    var sql = "INSERT INTO checkedOut(uid,itemName,qty,reason) VALUES";
    var values = "";
    if(Array.isArray(obj.qty)){
        for (let i = 0; i < obj.qty.length; i++) {
            if(i<(obj.qty.length-1)){
                values += "("+ "\'"+obj.uid+ "\'"+","+ "\'"+obj.itemName+ "\'"+"," +obj.qty[i] +"," + "\'"+ obj.reason[i] + "\'"+"),"
            }else{
                values += "("+ "\'"+obj.uid+ "\'"+","+ "\'"+obj.itemName+ "\'"+"," +obj.qty[i] +"," + "\'"+ obj.reason[i] + "\'"+")"
            }
        ...
        values += "("+ "\'"+obj.uid+ "\'"+","+ "\'"+obj.itemName+ "\'"+"," +obj.qty +"," + "\'"+obj.reason + "\'"+")"
    }
